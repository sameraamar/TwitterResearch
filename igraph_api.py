...
def drawGraph3D(graph, layout, angle, fileName, title = None):
	'''
	...
	vertexRadius =  (0.9 * 0.3) / numpy.sqrt(graph.vcount())
	...
	if title != None:
		con.set_font_size(36)
		drawer = igraph.drawing.text.TextDrawer(con, title, halign=TextDrawer.CENTER)
		drawer.draw_at(0, 40, width=600)
 
	# Draw the background
	con.set_source_rgba(0.2, 0.2, 0.2, 1.0)
